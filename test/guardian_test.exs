defmodule GuardianTest do
  ...
  describe "encode_and_sign" do
    ...
    test "encode_and_sign when after_encode_and_sign fails", ctx do
      assert {:error, :bad_things} = Guardian.encode_and_sign(ctx.impl, @resource, %{},
  ...
  describe "decode_and_verify" do
    ...
    test "verifying specific claims", ctx do
      claims = ctx.claims
      assert {:ok, ^claims} = Guardian.decode_and_verify(ctx.impl, ctx.token, %{some: "other_claim"})
    end

    test "a failing claim check", ctx do
      assert {:error, "not_a"} = Guardian.decode_and_verify(ctx.impl, ctx.token, %{not_a: "thing"})
    end

    test "failure on decoding", ctx do
      assert {:error, :decode_failure} = Guardian.decode_and_verify(ctx.impl, ctx.token, %{},
                                                                    [fail_decode_token: :decode_failure])
    end

    test "fails verifying within the token module", ctx do
      assert {:error, :verify_failure} = Guardian.decode_and_verify(ctx.impl, ctx.token, %{},
    ...
    test "fails on verify", ctx do
      assert {:error, :on_verify_failure} = Guardian.decode_and_verify(ctx.impl, ctx.token, %{},
  ...
  describe "revoke" do
    ...
    test "it fails before going to the impl if the token module fails", ctx do
      ...
      expected = [{Guardian.Support.TokenModule, :revoke, [ctx.impl, ctx.claims, ctx.token, [fail_revoke: :fails]]}]
  ...
  describe "refresh" do
    ...
    test "it fails before going to the impl if the token module fails", ctx do
      ...
      expected = [
        {Guardian.Support.TokenModule, :decode_token, [ctx.impl, ctx.token, [fail_refresh: :fails]]},
        {Guardian.Support.TokenModule, :verify_claims, [ctx.impl, ctx.claims, [fail_refresh: :fails]]},
  ...
  describe "exchange" do
    ...
    test "it fails before going to the impl if the token module fails", ctx do
      ...
      assert {:error, :fails} = Guardian.exchange(ctx.impl, ctx.token, claims["typ"], "refresh",
                                                  [fail_exchange: :fails])
